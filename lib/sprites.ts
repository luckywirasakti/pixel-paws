...
// Draw a pet centered in a square canvas context of the given pixel size.
export function drawPet(
  ...
  // pixel size so 16 cells * px ~= canvasSize * scale
  const px = Math.max(1, Math.floor((canvasSize * scale) / 16));
  const spriteSize = px * 16;
  const offX = Math.floor((canvasSize - spriteSize) / 2);
  const offY = Math.floor((canvasSize - spriteSize) / 2) + bob;

  for (let y = 0; y < 16; y++) {
    for (let x = 0; x < 16; x++) {
      const ch = rows[y][x];
      if (ch === ".") continue;
      const color = palette[ch] || palette.B;
      ctx.fillStyle = color;
  ...
  // Sunny variant sparkle
  if (variant === "sunny") {
    ctx.fillStyle = "#fff4c2";
    ctx.fillRect(offX + 13 * px, offY + 2 * px, px, px);
    ctx.fillRect(offX + 13 * px, offY + 4 * px, px, px);
    ctx.fillRect(offX + 12 * px, offY + 3 * px, px, px);
    ctx.fillRect(offX + 14 * px, offY + 3 * px, px, px);
  }
}
